...
async function runPhiloLit(questions) {
  ...
  return runMultipleChoiceQuiz(
    ...
    "You failed... \nI know it's tiring but would you like to try again? ..."
  );
}
...
async function runLikesGuess(questions) {
  ...
  for (const item of qs) {
    ...
    if (correctIndices == null) {
      if ("answer_index" in item) {
        ...
        correctIndices = Array.isArray(ai) ? ai : [ai];
      } else {
        correctIndices = [];
      }
    }
  ...
  append("You failed... \nWe know it's tiring but would you like to try again? ...\n");
  return false;
}
